        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h5>Tambah Inputan</h5>
                </div>
                <div class="card-body">
                    <form id="dynamicForm" action="{{ route('inputan.store') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control" id="client" name="input_berkas_id">
                                ...
                                @foreach($inputBerkass as $inputBerkas)
                                    <option value="{{ $inputBerkas->id }}" data-content="{{ $inputBerkas->nama_pemilik }}">{{ $inputBerkas->nama_pemilik }}-{{$inputBerkas->jenis_berkas}}-{{$inputBerkas->alamat}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" id="jenis_layanan" name="jenis_layanan_id">
                                ...
                                @foreach($jenisLayanans as $jenisLayanan)
                                    <option value="{{ $jenisLayanan->id }}" data-content="{{ $jenisLayanan->isi_inputan }}">{{ $jenisLayanan->jenis_layanan }}</option>
                                @endforeach
                            </select>
                        </div>
...
<style>
    ...
    .form-check {
        margin-top: 0px; /* Menambahkan margin di atas container checkbox */
    }
    .checkboxstyle {
        margin-bottom: 40px; /* Menghilangkan margin kiri pada checkbox */
    }
</style>
